...
db <- data.frame(readxl::read_excel("db.xlsx"))
...
db <- db %>% dplyr::mutate(
  ...
  VEA0 = saldo_deuda - saldo_aportes_sociales1,
...
s_libranza <- db_s_libranza %>% dplyr::select(c(1:8,12,16,17,20,56:61)) %>%
  dplyr::mutate(
    ...
    VIN2 = dplyr::if_else(as.POSIXlt(Sys.time())-db_s_libranza$fecha_vinculacion<=3600,1,0),
...
write.xlsx(x = df_list , file = "PE_COOPTRAISS_16_01_2025_v2_sin_descontar_aportes.xlsx", rowNames = FALSE)
